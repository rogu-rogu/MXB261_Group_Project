...
%   ∈[0,1] into n many subspaces of side length p (d-th root of n). It 
%   further divides the subspaces into intervals p^(d-1) and assigns one
%   sample to each subspace; such that the Latin Hypercube requirements are
%   satisfied for both samples within subspaces, and subspaces within the
%   overall space.
%
%   Input Arguments
...
%       columns contain the samples of each parameter.
...
samples = u_space;
% The dimensions of samples contain the parameter values generated by
% orthogonal sampling. For our assignment, we can treat this as 
% alpha = dim1, beta = dim2, rho = dim3